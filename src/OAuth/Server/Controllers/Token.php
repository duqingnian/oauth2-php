...
class Token
{
     ...
     * an authorization code from the Authorize Controller (@see OAuth\Server\Controllers\Authorize).
     * If the request is valid, an access token will be returned
     */
    public function token(Application $app)
    {
        // get the oauth server (configured in src/OAuth/Server/Server.php)
        $server = $app['oauth_server'];

        // get the oauth response (configured in src/OAuth/Server/Server.php)
